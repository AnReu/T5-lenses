...
# This file is based on the original implementation of the T5 model as part of huggingface transformers.
...
class T5LayerFF(nn.Module):
    ...
    def forward(self, hidden_states):
        activations = {}
        activations['before_layer_norm'] = hidden_states
        forwarded_states = self.layer_norm(hidden_states)
        activations['after_layer_norm'] = forwarded_states
        forwarded_states = self.DenseReluDense(forwarded_states)
        activations['after_dense_relu_dense'] = forwarded_states
        hidden_states = hidden_states + self.dropout(forwarded_states)
        activations['after_drop_out'] = hidden_states
...
class T5Block(nn.Module):
    ...
    def forward(
        ...
        hidden_states, activations = self.layer[-1](hidden_states)
...
class T5Stack(T5PreTrainedModel):
    ...
    def forward(
        ...
        if decoder_lens_layer is None:
            decoder_lens_layer = len(self.block) - 1
        for i, layer_module in enumerate(self.block):
            if (not self.is_decoder) and (i > decoder_lens_layer):
                break
...
class T5Model(T5PreTrainedModel):
    ...
    def forward(
        ...
        decoder_lens_layer: Optional[int] = None,
        ...
        if encoder_outputs is None:
            encoder_outputs = self.encoder(
                ...
                return_dict=return_dict,
                decoder_lens_layer=decoder_lens_layer,
            )
